// 更新公告数据 - 面向玩家的简化版本
// 倒序排列（最新版本在前）
const CHANGELOG = [
    {
        version: "7.08",
        date: "2026-05-01",
        title: "营地摊位细节优化",
        highlights: [
            "修复营地环境物可能遮住摊位的问题",
            "精修摊位外观，新增顶棚、地毯和货台细节",
            "空摊位标识更清晰，集市区域更容易辨认"
        ]
    },
    {
        version: "7.07",
        date: "2026-05-01",
        title: "罗格营地视觉升级",
        highlights: [
            "优化营地地面层次，加入道路、广场和集市区域细节",
            "强化营地边缘墙体阴影和轮廓，安全区不再像平铺地砖",
            "新增火把、木箱、木桶、旗帜等营地环境物",
            "营地光源和前景遮挡更自然，整体更有生活感"
        ]
    },
    {
        version: "7.06",
        date: "2026-05-01",
        title: "地牢氛围与空间层次升级",
        highlights: [
            "新增地牢前景环境物，角色穿行时有更自然的前后遮挡",
            "强化森林、冰窟和地狱的房间主题，地图辨识度更高",
            "加入火光、冷光和微弱环境光，探索氛围更接近暗黑地牢",
            "优化场景物件与房间结构的摆放，减少重复感"
        ]
    },
    {
        version: "7.05",
        date: "2026-05-01",
        title: "地牢地图视觉升级",
        highlights: [
            "重做地牢墙体边缘和转角表现，地图轮廓更清楚",
            "新增房间地面结构和环境层次，高层地牢不再大片重复",
            "优化木桶、木箱、陶罐等场景物件摆放，减少贴墙和嵌墙",
            "调整地面装饰尺寸与密度，探索画面更干净耐看"
        ]
    },
    {
        version: "7.04",
        date: "2026-05-01",
        title: "战斗手感与精英怪优化",
        highlights: [
            "优化物理攻击、暴击和击杀音效，战斗反馈更清晰",
            "精英怪词缀表现更完整，诅咒、多重射击等效果更容易辨认",
            "怪物行为进一步区分，沉沦魔、僵尸、骷髅和木乃伊更有特点",
            "优化精英怪初始刷新逻辑，进入地牢时也会出现完整词缀怪"
        ]
    },
    {
        version: "7.03",
        date: "2026-05-01",
        title: "地图、怪物与主角焕新",
        highlights: [
            "地图场景加入更多层次和环境细节，地牢氛围更丰富",
            "多种怪物贴图重新绘制，轮廓更清晰，辨识度更高",
            "主角贴图与移动动画优化，左右和斜向移动更自然",
            "优化世界频道公告展示，减少重复刷屏"
        ]
    },
    {
        version: "6.99",
        date: "2026-01-11",
        title: "挂机平衡 & 套装缩放",
        highlights: [
            "私聊功能：点击玩家名或输入@玩家名发起私聊",
            "世界频道：快捷表情面板，一键发送常用表情",
            "聊天框宽高放大，适配手机竖屏",
            "挂机模式：套装掉率20%，暗金掉率40%",
            "套装属性随层数增强（攻速、伤害%、暴击伤害等）"
        ]
    },
    {
        version: "6.98",
        date: "2026-01-10",
        title: "世界频道物品分享",
        highlights: [
            "装备tooltip新增「分享到世界频道」按钮",
            "聊天中的物品链接可点击查看详细属性",
            "物品链接按稀有度显示对应颜色"
        ]
    },
    {
        version: "6.97",
        date: "2026-01-10",
        title: "战斗系统大修",
        highlights: [
            "修复护盾被部份怪物穿透",
            "修复连锁闪电正确触发掉落、经验、成就",
            "护甲公式改为百分比减伤(100护甲=50%减伤)",
            "卖出套装、强化装备时需要二次确认"
        ]
    },
    {
        version: "6.96",
        date: "2026-01-06",
        title: "长时间挂机性能优化",
        highlights: [
            "小地图缓存系统：只在探索新区域时重绘，挂机时 0 开销",
            "物品标签优化：摄像机静止时跳过 DOM 更新",
            "遮挡修复优化：复用 Set + 位运算编码，消除 GC 压力"
        ]
    },
    {
        version: "6.95",
        date: "2026-01-06",
        title: "称号商店系统",
        highlights: [
            "神秘贤者新增称号商店，7种称号（1万~5亿金币）",
            "购买仪式感：金币扣除动画 + 称号获得特效弹窗",
            "100万以上称号购买时全服公告，称号显示在聊天/头顶"
        ]
    },
    {
        version: "6.94",
        date: "2026-01-05",
        title: "高等级经验优化",
        highlights: [
            "怪物经验改为指数增长，高层刷怪效率大幅提升",
            "40级以上玩家升级速度提升2-4倍",
            "修复护盾技能第3阶段无法点亮的问题，增加护盾音效"
        ]
    },
    {
        version: "6.93",
        date: "2026-01-03",
        title: "自动战斗雇佣费",
        highlights: [
            "开启自动战斗时收取15%金币雇佣费",
            "首次开启时弹出提醒面板（游戏暂停）"
        ]
    },
    {
        version: "6.92",
        date: "2026-01-03",
        title: "死亡复活系统",
        highlights: [
            "死亡弹窗：显示战绩统计（层数、击杀、等级、死因）",
            "原地复活：消耗金币复活，费用随等级/层数增长",
            "安全复活：远离敌人的安全位置+1.5秒无敌 + 满血满蓝"
        ]
    },
    {
        version: "6.91",
        date: "2026-01-03",
        title: "自动战斗拾取优化",
        highlights: [
            "自动战斗时装备远距离拾取：和金币药水一样",
            "彻底解决装备在墙角导致的'无法到达'问题"
        ]
    },
    {
        version: "6.9",
        date: "2026-01-02",
        title: "增加护盾技能",
        highlights: [
            "增加护盾技能和技能树",
            "护盾视觉效果：金色椭圆光环 + 血球金边",
            "自动战斗支持：血量低于50%自动释放护盾",
            "雷电术增强：阶段2/3多落雷(1→2→4根)，优先攻击群怪"
        ]
    },
    {
        version: "6.8",
        date: "2026-01-02",
        title: "技能树系统",
        highlights: [
            "技能树重构：每个技能3个阶段，第2/3阶段可选择分叉路线",
            "分叉选择：选择后锁定路线，需洗点才能重选",
            "老存档迁移：自动转换技能等级，多余点数返还"
        ]
    },
    {
        version: "6.7",
        date: "2026-01-01",
        title: "图鉴系统",
        highlights: [
            "套装图鉴：收集套装装备，追踪套装完成度",
            "怪物图鉴：记录击杀过的怪物和BOSS，统计击杀数"
        ]
    },
    {
        version: "6.6",
        date: "2025-12-30",
        title: "深渊挑战系统",
        highlights: [
            "开启深渊挑战：无限层级，每周重置榜单发放奖励",
            "深渊专属奖励：深渊征服者6件套装及专属称号"
        ]
    },
    {
        version: "6.5",
        date: "2025-12-25",
        title: "Boss技能系统 & 周榜系统",
        highlights: [
            "boss 增强技能：远距、毒箭、狂暴、召唤小怪、扇形、多方向等",
            "新增周榜：每周一重置，新玩家也能冲榜！"
        ]
    },
    {
        version: "6.4",
        date: "2025-12-24",
        title: "多端云同步 & UI精修",
        highlights: [
            "上线多端云同步：同步码6位，支持多端登录",
            "多设备互踢：跨浏览器/跨设备登录自动检测并接管会话",
            "界面精修：引入 gsap 库，优化动画效果"
        ]
    },
    {
        version: "6.3.2",
        date: "2025-12-22",
        title: "存档保护 & 装备需求平衡",
        highlights: [
            "🛡️存档保护：修复极端情况下快速点击可能导致存档丢失的问题",
            "修复套装/暗金装备需求过高无法装备的问题"
        ]
    },
    {
        version: "6.3.1",
        date: "2025-12-22",
        title: "安全修复 & Boss难度升级",
        highlights: [
            "技能交互改进：未学习的技能按钮灰色禁用，点击提示引导",
            "注册流程优化：更新公告和昵称注册按顺序显示，避免重叠",
            "昵称安全升级：集成聊天敏感词库，包含敏感词时阻止注册并提示",
            "BOSS升级：全面提升血量和伤害，掉落升级数量"
        ]
    },
    {
        version: "6.3.0",
        date: "2025-12-21",
        title: "成就系统 & 离线收益",
        highlights: [
            "成就扩展：从6个增加到30个，涵盖击杀、探索、收集、战斗、经济、成长六大类别",
            "成就面板优化：完美适配电脑和直屏手机",
            "增加离线收益系统"
        ]
    },
    {
        version: "6.2",
        date: "2025-12-21",
        title: "打击感革命 & 场景破坏系统",
        highlights: [
            "场景破坏：地牢新增可破坏的木桶、木箱和陶罐",
            "受击顿帧、实体缩放、屏幕震动反馈，可在通用设置开启",
            "物理掉落系统：物品掉落现在拥有抛物线轨迹、落地弹跳",
            "阶梯式音效：受击音效现在分为普通/暴击/击杀",
            "掉落机制重塑：仅套装永久保留，其他分阶梯消失",
            "视觉增强：物理血溅、物理击退"
        ]
    },
    {
        version: "6.1.1",
        date: "2025-12-20",
        title: "体验优化 & 拾取增强",
        highlights: [
            "自动战斗优化：视金钱如套装！金币拾取优先级提至最高，且支持隔墙侦测",
            "视觉增强：大幅提升经验条百分比文字清晰度（白色+黑边阴影）",
            "精度修补：修复击杀怪物时漂浮经验值出现多位小数的浮点误差问题",
            "智能脱困：增加10秒位置位移监控，自动放弃并拉黑无法到达的掉落物，防止挂机卡死"
        ]
    },
    {
        version: "6.1",
        date: "2025-12-19",
        title: "性能重构 & 视觉升级",
        highlights: [
            "深度性能优化：重构AI计算内核，CPU占用大幅降低",
            "渲染优化：离屏Canvas绘制血迹，后期战斗帧率翻倍",
            "元素视觉增强：冰冷/闪电伤害拥有专属受击光效和数字特效",
            "物品栏优化：需求不足时保留稀有度边框，一眼辨识神器",
            "关键修复：基格商人库存同步修复，AI距离计算修正"
        ]
    },
    {
        version: "6.0",
        date: "2025-12-18",
        title: "每日登录 & 商店扩展",
        highlights: [
            "每日登录重做：7天全是buff奖励",
            "Day7：24小时三倍经验 + 套装装备",
            "商店新增双倍经验卷轴（1000G/1小时）",
            "地图放大：64×64 → 80×80（+56%探索空间）",
            "走廊加宽：至少2格宽，自动战斗不再卡住",
            "修复：每日任务Boss不计数"
        ]
    },
    {
        version: "5.9",
        date: "2025-12-17",
        title: "玩家摆摊系统",
        highlights: [
            "玩家摆摊：在罗格营地可摆摊出售装备",
            "摊位费：500G/小时，最多10小时",
            "离线摆摊：关闭游戏也能卖货",
            "实时交易：商品售出即时通知",
            "修正：内嵌背包金币显示"
        ]
    },
    {
        version: "5.8",
        date: "2025-12-16",
        title: "移动端适配",
        highlights: [
            "适配移动端（含直屏）",
            "商店/仓库/锻造面板内嵌背包",
            "背包/仓库一键整理功能",
            "取消入口提示和 Enter 键进入",
            "自动战斗时物品丢弃 5 秒内不再拾取",
            "装备属性对比改为双栏气泡更直观"
        ]
    },
    {
        version: "5.7",
        date: "2025-12-14",
        title: "每日任务 & 仓库扩建",
        highlights: [
            "每日任务系统重做：链式解锁（简单→中等→困难）",
            "任务目标根据等级动态计算，困难任务奖励技能点",
            "仓库扩建：花费金币扩容，最多3次（36→54格）",
            "任务/公告/成就统一显示楼层名（如「枯木墓地」）"
        ]
    },
    {
        version: "5.6",
        date: "2025-12-14",
        title: "社交：世界聊天 & 优化",
        highlights: [
            "新增世界聊天频道，实时畅聊",
            "地牢每层都有独特名称（森林/冰原/熔岩三大群系）",
            "传送门支持选择任意已到达层数",
            "吸血鬼AI：突进吸血攻击",
            "闪电幽魂AI：发射闪电球+穿墙移动",
            "修复远程怪物隔墙攻击的问题",
        ]
    },
    {
        version: "5.5",
        date: "2025-12-14",
        title: "自动战斗优化 & 性能优化",
        highlights: [
            "激烈战斗判定：改为任意敌人80内，不会忘记拣金币",
            "地图离屏Canvas缓存绘制，性能提升 ~5800 倍",
            "排行榜：增加富豪榜"
        ]
    },
    {
        version: "5.4",
        date: "2025-12-14",
        title: "性能优化 & 画质设置",
        highlights: [
            "新增画质设置：华丽特效/性能优先 可选",
            "移除高频震屏（普通攻击/暴击/被击中等）",
            "粒子上限200个，优化低配电脑性能",
            "自动战斗：打开面板不再暂停"
        ]
    },
    {
        version: "5.3",
        date: "2025-12-13",
        title: "全服公告系统",
        highlights: [
            "顶部滚动公告栏：实时显示全服玩家动态",
            "击杀Boss全服通报（金色）",
            "获得套装全服通报（绿色）"
        ]
    },
    {
        version: "5.2",
        date: "2025-12-13",
        title: "世界细节重塑",
        highlights: [
            "全新地图装饰系统：告别单调墙壁",
            "三大群系专属装饰：森林古树、冰原水晶、地狱尖塔",
            "混合渲染技术：程序化细节 + 像素风贴图完美融合"
        ]
    },
    {
        version: "5.1",
        date: "2025-12-13",
        title: "视觉效果更新",
        highlights: [
            "怪物死亡地面血迹效果，15-25秒渐隐消失",
            "技能视觉优化：闪电链视觉增强、火球增加拖尾粒子、多重射击箭矢及拖尾",
            "Boss和精英怪增加死亡特效",
            "升级特效：金色闪光+光柱+粒子+震屏",
            "物品吸入效果：贝塞尔曲线飞行拾取",
            "回城卷轴增加仪式感",
            "低血量和加点时增加音效",
            "暴击反馈增强：慢动作+金色数字+更强打击感",
            "连击系统：显示连击数（纯视觉爽感，无数值影响）"
        ]
    },
    {
        version: "5.0",
        date: "2025-12-13",
        title: "多存档系统",
        highlights: [
            "支持3个独立存档，可同时培养多个角色",
            "增加新手引导",
            "每日登录奖励增加领取特效"
        ]
    },
    {
        version: "4.9",
        date: "2025-12-12",
        title: "怪物系统大扩展",
        highlights: [
            "新增6种怪物：僵尸、骷髅战士、幽灵鬼魂、闪电幽魂、木乃伊、吸血鬼",
            "幽灵可穿墙+30%闪避，木乃伊中毒攻击，吸血鬼20%吸血",
            "怪物按层数逐步解锁，权重池随机生成"
        ]
    },
    {
        version: "4.8",
        date: "2025-12-11",
        title: "铁匠铺强化系统",
        highlights: [
            "新增NPC恰西，提供装备强化服务",
            "消耗2件同部位同品质装备强化，最高+9",
            "+6以上有失败风险，可能降级"
        ]
    },
    {
        version: "4.7",
        date: "2025-12-11",
        title: "游戏优化",
        highlights: [
            "代码架构重构，性能提升",
            "常量系统统一管理"
        ]
    },
    {
        version: "4.6",
        date: "2025-12-10",
        title: "数值平衡",
        highlights: [
            "天神赐福每种最多获得3次",
            "天赋商店刷新价格递增",
            "传奇天赋第5层后解锁",
            "Day3登录奖励改为24小时双倍经验"
        ]
    },
    {
        version: "4.5",
        date: "2025-12-10",
        title: "冰冻系统优化",
        highlights: [
            "冰冻硬控时间2秒→0.5秒",
            "新增1.5秒减速期，可逃跑喝药",
            "冰冻免疫时间3秒→5秒",
            "冰冻精英怪头顶显示❄️图标"
        ]
    },
    {
        version: "4.4",
        date: "2025-12-10",
        title: "死亡系统优化",
        highlights: [
            "死亡时画面变灰，5秒倒计时后回城",
            "自动战斗：没有回城卷时不再重复尝试"
        ]
    },
    {
        version: "4.3",
        date: "2025-12-08",
        title: "掉落特效",
        highlights: [
            "暗金装备掉落：金色光柱+震屏",
            "套装装备掉落：绿色光柱+神秘音效"
        ]
    },
    {
        version: "4.2",
        date: "2025-12-08",
        title: "每日登录奖励",
        highlights: [
            "7天循环奖励，Day7送暗金装备",
            "掉落率大幅下调，提升稀缺感"
        ]
    },
    {
        version: "4.1",
        date: "2025-12-08",
        title: "天神赐福优化",
        highlights: [
            "新增多种赐福类型",
            "赐福面板可查看已获得列表"
        ]
    },
    {
        version: "4.0",
        date: "2025-12-08",
        title: "天赋商店 & 套装扩展",
        highlights: [
            "每层进入时可购买天赋强化角色",
            "套装从3套扩展到9套（54件装备）"
        ]
    },
    {
        version: "3.9",
        date: "2025-12-08",
        title: "属性系统简化",
        highlights: [
            "装备属性直接显示效果（如+50%伤害）",
            "地面物品名称移至图标上方"
        ]
    },
    {
        version: "3.8",
        date: "2025-12-08",
        title: "性能优化 & 掉落重平衡",
        highlights: [
            "敌人对象池系统，减少卡顿",
            "层数加成、累积幸运机制",
            "每8只怪必掉1个消耗品"
        ]
    },
    {
        version: "3.7",
        date: "2025-12-08",
        title: "排行榜 & 技能减负",
        highlights: [
            "排行榜实时更新",
            "所有技能法力消耗降低",
            "传送门支持层数选择"
        ]
    },
    {
        version: "3.6",
        date: "2025-12-07",
        title: "无限任务 & 技能图标",
        highlights: [
            "完成10个任务后任务系统重置循环",
            "技能图标改为像素风格精灵图"
        ]
    },
    {
        version: "3.5",
        date: "2025-12-07",
        title: "自动战斗优化",
        highlights: [
            "低血量敌人优先击杀",
            "近战不再误判为卡墙"
        ]
    },
    {
        version: "3.4",
        date: "2025-12-07",
        title: "无限层级BOSS",
        highlights: [
            "11层开启二周目，BOSS无限循环",
            "每周目BOSS血量+150%，伤害+60%"
        ]
    },
    {
        version: "3.3",
        date: "2025-12-06",
        title: "战斗体验优化",
        highlights: [
            "普攻显示斩击弧特效",
            "自动战斗：智能拾取、目标锁定"
        ]
    },
    {
        version: "3.2",
        title: "自动战斗设置",
        highlights: [
            "设置自动保存",
            "营地无法开启自动战斗"
        ]
    },
    {
        version: "3.1",
        title: "自动战斗系统",
        highlights: [
            "按F键开启自动战斗",
            "A*寻路、自动拾取、自动喝药",
            "紧急回城、防卡死机制"
        ]
    },
    {
        version: "3.0",
        title: "套装系统",
        highlights: [
            "新增3套套装（法师/战士/刺客）",
            "2/4/6件套提供阶段性加成",
            "套装相关成就"
        ]
    },
    {
        version: "2.9",
        title: "火球术进化",
        highlights: [
            "Lv5解锁爆炸效果",
            "爆炸范围和伤害随等级提升"
        ]
    },
    {
        version: "2.8",
        title: "地狱模式修复",
        highlights: [
            "修复地狱层数管理和传送逻辑"
        ]
    },
    {
        version: "2.7",
        title: "洗点系统",
        highlights: [
            "新增神秘贤者NPC",
            "可重置属性点和技能点"
        ]
    },
    {
        version: "2.6",
        title: "技能体系简化",
        highlights: [
            "移除玩家冰霜伤害，保留抗性"
        ]
    },
    {
        version: "2.5",
        title: "雷电术闪电链",
        highlights: [
            "Lv2+解锁闪电链，可跳跃多个目标"
        ]
    },
    {
        version: "2.3",
        title: "新技能：雷电术",
        highlights: [
            "替换冰霜新星，召唤闪电打击敌人"
        ]
    },
    {
        version: "2.1",
        title: "地狱模式",
        highlights: [
            "击败巴尔解锁地狱模式",
            "怪物血量×6，伤害×4，经验×5"
        ]
    },
    {
        version: "2.0",
        title: "核心玩法大更新",
        highlights: [
            "抗性系统（火/冰/雷/毒）",
            "40+种装备词缀",
            "12种精英怪词缀",
            "装备等级需求系统"
        ]
    },
    {
        version: "1.8",
        title: "游戏机制优化",
        highlights: [
            "点击远处物品自动走过去拾取",
            "BOSS血量和伤害大幅增强",
            "修复隔墙攻击问题"
        ]
    },
    {
        version: "1.7",
        title: "成就系统",
        highlights: [
            "6个精心设计的成就",
            "实时进度追踪"
        ]
    },
    {
        version: "1.6",
        title: "音效与平衡",
        highlights: [
            "药剂音效、箭矢音效",
            "技能射程限制",
            "怪物名称血条优化"
        ]
    },
    {
        version: "1.5",
        date: "2025-11-26",
        title: "仓库系统",
        highlights: [
            "找瓦瑞夫存取物品",
            "任务扩展至10个",
            "背包物品可丢弃",
            "少量贴图"
        ]
    },
    {
        version: "1.0",
        date: "2025-11-25",
        title: "基本功能开发",
        highlights: [
            "城镇、NPC、随机地牢、回程卷轴、物品栏",
            "点数、装备、攻击、技能、传送门、boss、自动拾取金币"
        ]
    }
];

// 获取当前版本号
const CURRENT_VERSION = CHANGELOG[0].version;

// 初始化版本号显示
document.addEventListener('DOMContentLoaded', () => {
    const versionEl = document.getElementById('game-version');
    if (versionEl) versionEl.textContent = 'v' + CURRENT_VERSION;
});
