...
### 命中减噪与物理立体特效（2026-09-08）

同目标140毫秒内同色伤害合并显示、保留总和，暴击单独显示；移除重复暴击数字、天赋词条、横扫名称、击杀经验与重复破绽飘字。常驻战术说明仅显示自动战斗当前目标，主动危险预警和破绽保留，打断进度只在有实际进度时出现。

普通命中与多重箭使用最多5个（低画质2个）三维旋转的四面体碎片，按面光照投影到Canvas；不新增GPU上下文或图片下载。高阶物理横扫用三维弧形刀面、侧壁和残影投影，暴击增加亮锋，最多同时6次，性能模式保持原弧线。移除普攻额外点爆与白色暴击圆形爆闪，陨石/雷暴专属效果保持。

全量验证106秒通过；同目标合并总和/暴击隔离/不同目标隔离、几何有限坐标、数量上限和低画质回退测试通过。真实performAttack触发4目标群体横扫，命中后事件清理为0、浏览器运行错误0。桌面截图 `C:/Users/voroj/.codex/visualizations/2026/09/06/01a076cc-6a6b-7c00-900a-1eeddbea931a/physical-depth.png`；手机尺寸验证为桌面GPU，未测真机持续性能。

## 2026-09-08 残留点爆复查

清理多重箭出手、拖尾、撞墙/命中重复圆点，以及近战旧图集的圆形爆闪；森林萤火降低密度和亮度，通用粒子绘制遵守 maxAlpha。常驻战术说明移除，即时预警与破绽保留。

| 验收 | 证据 | 结果 |
| --- | --- | --- |
| 箭矢撞墙不额外喷点、环境透明度和淡出 | tools/test-hit-feedback.js | 通过 |
| 名称旁无常驻说明、预警/打断/破绽仍生效 | tools/test-combat-tactics.js | 通过 |
| 实际普攻群体横扫 | qa.html 立体裂斩：4目标均受伤、效果到时清理、页面错误0 | 通过 |

会话截图 dots-before.png、dots-after.png、dots-final.png 位于 C:/Users/voroj/.codex/visualizations/2026/09/06/01a076cc-6a6b-7c00-900a-1eeddbea931a/。音效仅准备授权素材候选，未改游戏音频。

本轮完整检查88项通过；旧成长效果测试的重复爆发要求已按新行为更新，失败项复跑通过。相关命中、战术、物理横扫和技能实际伤害回归均通过。
