...
旧移动分支只判断角色中心所在格：半径12的角色可走到距墙1px处；大步长还可能越过整格墙。现在使用圆与墙格矩形的相交检测，每步不超过6px，按轴滑动。非法旧落脚点只在移动开始时就近恢复；冻结时保留移动目标。

| 验收 | 结果 | 证据 |
|---|---|---|
| 自动回归 | 原代码红测：x=119时已压墙；修复后墙边、跨格、墙角、沿墙滑动、边界、窄走廊和旧位置脱离通过 | `tools/test-player-wall-collision.js` |
| 桌面实际update移动 | 墙x=1040、角色x=1026，间距14≥半径12，PASS | `wall-collision-desktop.json` / `.png` |
| 390px触控路径模拟 | 墙x=1400、角色x=1386，间距14≥半径12，PASS | `wall-collision-mobile.json` / `.png` |

浏览器运行错误0、未处理Promise0。上述证据位于 `C:/Users/voroj/.codex/visualizations/2026/09/06/01a076cc-6a6b-7c00-900a-1eeddbea931a/`；QA按钮“实测向墙内移动”在真实生成地图上设置墙内目标，经正式update路径完成移动。声音未修改，移动端验证为浏览器模拟而非实体手机。

统一验证79项通过（359秒），日志 `wall-verify.txt`。验收页与临时服务器已关闭；纯前端更新无需后端重启。
