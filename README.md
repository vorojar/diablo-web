...
- **技能树** (v6.8 分叉系统):
  - 每个技能3个阶段：基础(Lv1-5) → 分叉选择(Lv1-5) → 终极分叉(Lv1-5)
  - 🔥 **火焰系 (Q)**: 火球术 → 爆炸强化/灼烧 → 流星/火焰新星/蔓延/引爆
  - ⚡ **雷电系 (W)**: 雷电术 → 闪电链/雷暴 → 超载/瘫痪/风暴/雷霆之怒
  - 🏹 **射击系 (E)**: 多重射击 → 穿透箭/散射 → 贯穿/爆裂/弹幕/连珠
  - 🛡️ **护盾系 (R)** (v6.9): 神圣护盾 → 反射护盾/持久护盾 → 守护光环/绝对防护
...
- **套装系统** (10套60件):
  ...
  - 🟢 **格里斯沃尔德的传承**: 圣骑士套装，强化防御 and 生存（6件: 伤害+300%, 防御+200）
  - 🟢 **庄·欧的化身**: 死灵法师套装，强化毒素 and 法力（6件: 毒伤+300, 伤害+100%）
  - 🟢 **奥杜尔的节拍**: 德鲁伊套装，强化生命恢复（6件: HP+400, 伤害+200%）
  - 🟢 **马维娜的战斗颂歌**: 狂战士套装，强化狂暴伤害（6件: 伤害+400%, 暴击+25%）
  - 🟢 **希冈的钢铁**: 混沌套装，全属性均衡提升（6件: 全属性大幅提升）
  - 🟢 **深渊征服者**: 深渊专属奖励（6件: 伤害+500%, 暴击+15%, 吸血+5%, 攻速+20%）
...
### 怪物与战斗系统
- **AI类型**: 近战追击、远程攻击、复活友军（巫师）、穿墙追击（幽灵）、任务BOSS。
- **怪物种类** (9种普通怪物 + 6种BOSS):
  - **沉沦魔** (全层): 基础近战怪物
  - **僵尸** (1+层): 高血量、低速度、低伤害的坦克型
  - **骷髅弓箭手** (2+层): 远程怪物，发射可视箭矢攻击
  - **骷髅战士** (2+层): 快速近战怪物
  - **沉沦魔巫师** (3+层): 可以复活死去的怪物
  - **幽灵鬼魂** (4+层): 🔮穿墙移动 + 🛡️30%闪避
  - **闪电幽魂** (5+层): ⚡闪电球攻击 + 🔮穿墙移动
  - **木乃伊** (6+层): ☠️中毒攻击（持续3秒DOT）
  - **吸血鬼** (7+层): 🩸突进吸血攻击（冲刺后吸取生命）
  - **精英怪** (10%概率): 任意怪物的强化版，橙色名称
  - **任务BOSS** (特定层): 血鸟、女伯爵、屠夫、树头木拳、暗黑破坏神、巴尔
- **Boss技能系统** (v6.5新增):
  - 🏹 **血鸟** (2层): 远程AI + 5支毒箭齐射
  - 🔥 **女伯爵** (4层): 传送闪现 + 火焰新星
  - 🗡️ **屠夫** (5层): 远距突进 + 低血狂暴(30%血量触发，攻速+50%伤害+30%)
  - 🌍 **树头木拳** (7层): 地震波(范围减速) + 召唤小怪
  - ⚡ **暗黑破坏神** (9层): 扇形闪电吐息 + 双元素攻击
  - 🐙 **巴尔** (10层): 4方向触手攻击 + 范围冰冻
- **怪物显示**: 所有怪物显示名称，颜色区分稀有度（Boss红色、精英橙色、普通灰白），血条在怪物头顶。
- **权重池生成**: 怪物按层数逐步解锁，使用权重随机选择，保证多样性。
...
- **v4.2** - 📅 **每日登录奖励 & 掉落率平衡**:
  - **每日登录奖励系统**:
    - 7天循环奖励：Day1 金币 → Day2 药水 → Day3 双倍经验 → Day7 暗金装备
    - 首次当日登录自动弹出奖励面板
    - Day7 特殊金色光效动画
    - 连续登录天数记录和显示
  - **掉落率大幅下调**（提升稀缺感）:
...
- **v4.5** - ❄️ **冰冻系统优化**:
  - **硬控时间大幅缩短**: 冰冻硬控从 2秒 → **0.5秒**（完全无法移动）
  - **新增减速期**: 硬控结束后进入 **1.5秒减速期**（移动速度40%），玩家可以缓慢逃跑或喝药
  - **免疫时间延长**: 从 3秒 → **5秒**，面对多个冰冻怪时更有喘息空间
  - **视觉警告**: 拥有冰冻技能的精英怪头顶显示 **❄️** 图标，提前预警威胁
  - **设计理念**: 将"2秒站桩挨打必死"改为"0.5秒眩晕+1.5秒慢动作可求生"，大幅提升自动战斗生存率

- **v4.6** - ⚖️ **数值平衡 & 系统优化**:
  - **🌟 天神赐福系统平衡**:
    - **叠加上限**: 每种赐福最多获得 **3次**（原无上限），防止长期数值膨胀
    - **稀有率调整**: 基础稀有率从 30% → **15%**，但加入 **5次保底机制**（连续5次普通后必出稀有）
    - **设计理念**: 稀有更稀有，但保底让玩家不会永远非酋
  - **🏪 天赋商店机制改进**:
    - **刷新价格递增**: 30→60→120→240...（每次翻倍），防止无限刷新寻找完美Build
    - **传奇天赋解锁**: 玻璃大炮、凤凰等传奇天赋只在 **第5层后** 出现
    - **传奇天赋涨价**: 玻璃大炮 250→**500**金，凤凰 500→**1000**金
    - **防刷商店修复**: 只有进入 **更高层** 才触发商店（回到低层再进不会重复触发）
  - **📅 每日登录奖励调整**:
    - **Day3 奖励变更**: 「赐福机会+1」→ 「**24小时双倍经验**」
    - **设计理念**: 临时buff鼓励实际游玩，而非混登录获取永久加成
  - **⚡ 双倍经验Buff**:
    - 领取后24小时内击杀怪物经验翻倍
    - 经验值显示为双倍数值（如 +30 XP）
...
- **v4.8** - ⚒️ **铁匠铺强化系统**:
  ...
  - **强化机制**:
    ...
    - 最高可强化至+9
  - **风险收益**:
    - +0→+5: 100%~50%成功率，失败无惩罚
    - +6→+9: 40%~20%成功率，失败50%概率降级
  - **一键填充**: 自动从背包搜索符合条件的祭品装备
  - **华丽特效**:
    - 成功: 金色粒子爆发 + 全屏闪光 + 暗金掉落音效
    - 失败: 灰色烟尘 + 撞击音效
  - **🩹 Bug修复**:
    - 修复点击NPC时面板闪烁问题（改用单次点击检测替代持续按住检测）

- **v4.9** - 👹 **怪物系统大扩展**:
  - **新增6种怪物**:
    - **僵尸** (1+层): 高血量(1.5x)、低速度、低伤害的坦克型
    - **骷髅战士** (2+层): 快速近战怪物
    - **幽灵鬼魂** (4+层): 🔮穿墙移动 + 🛡️30%闪避
    - **闪电幽魂** (5+层): ⚡闪电球攻击 + 🔮穿墙移动
    - **木乃伊** (6+层): ☠️中毒攻击，持续3秒DOT
...
- **v5.0** - 💾 **多存档系统**:
  - **三存档槽位**: 支持3个独立存档，可同时培养多个角色
  - **新手引导**: 增加新手引导系统
  - **每日登录优化**: 领取特效增强

- **v5.1** - ✨ **Game Juice特效大更新**:
  - **🩸 地面血迹系统**:
    - 怪物死亡时在地面留下不规则血迹
    - 血迹持续15-25秒后渐隐消失
    - 最多100个血迹，防止性能问题
  - **⚡ 闪电链视觉增强**:
    - 三层渲染：外层蓝色光晕 + 中层主体 + 白色内核
    - 40%概率产生分叉闪电
    - 蓝色残影效果（比主闪电更持久）
    - 命中点火花散射
  - **🔥 火球拖尾粒子**:
  ...
  - **🔊 UI音效反馈**:
    ...
    - 金币连续拾取时音调递增（马里奥风格）
  - **💥 暴击反馈增强**:
    - 暴击时进入微慢动作（0.1s/50%速度），增加打击感
    - 暴击震屏效果增强（6强度）
    - 金色伤害数字（带爆炸emoji）
    - 暴击时斩击特效更华丽（金色+发光+更多弧线）
    - 屏幕粒子爆发特效
  - **🔥 连击系统**:
    - 增加Combo连击计数器（2.5秒窗口）
    - 连击数越高，计数器颜色越华丽（黄->橙->红->紫）
    - 纯视觉爽感反馈，不影响数值平衡

- **v5.2** - 🏰 **世界细节重塑**:
  - **地图装饰系统**: 告别单调墙壁，增加环境细节
  - **三大群系专属装饰**:
    - 🌲 森林群系: 古树、藤蔓、蘑菇
    - ❄️ 冰原群系: 冰晶、水晶簇
    - 🔥 地狱群系: 尖塔、熔岩裂缝
...
- **v5.3** - 📢 **全服公告系统**:
  - **顶部滚动公告栏**: 实时显示全服玩家动态
  - **Boss击杀通报**: 金色公告，击杀Boss后全服可见
  - **套装掉落通报**: 绿色公告，获得套装后全服可见
  - **Realtime实时推送**: 基于PocketBase WebSocket，毫秒级延迟
  - **降级兼容**: 订阅失败自动回退30秒轮询模式

- **v5.4** - ⚡ **性能优化 & 画质设置**:
  - **🎨 画质设置系统**:
    - 新增「通用设置 → 特效质量」选项
    - **华丽特效**（默认）: 全部发光效果开启
    - **性能优先**: 关闭日常发光特效，只保留暗金/套装掉落光柱
    - 低配电脑玩家可切换至性能模式，大幅减少GPU开销
    - 设置即时生效，无需重启游戏
  - **📉 震屏频率优化**:
    - 移除普通攻击震屏
    - 移除暴击震屏
    ...
    - 移除投射物命中震屏
    - 移除大伤害震屏
    - 只保留Boss/精英死亡、暗金掉落等稀有时刻的震屏
  - **🎯 慢动作优化**:
    - 移除暴击慢动作，只保留Boss/精英死亡慢动作
  - **🔢 粒子上限**:
    - 全局粒子数量上限200个，超出不创建
  - **🤖 自动战斗改进**:
    - 打开面板（物品、技能、任务等）时不再暂停自动战斗
    - 可以边查看装备边挂机

- **v5.5** (2025-12-12) - 🤖 **自动战斗优化 & 性能优化**:
  - **激烈战斗判定优化**: 改为任意敌人80内即视为激烈战斗
  - **拾取策略更智能**: 战斗时优先打怪，脱战后再拾取
  - **性能优化**: 性能优先模式：粒子数量减半，拖尾密度降低 60%
  - **性能优化**: 地图离屏Canvas缓存绘制，性能提升 ~5800 倍
  - **排行榜**: 增加富豪榜

- **v5.6** (2025-12-13) - 💬 **世界聊天 & 地牢命名**:
  - **世界聊天系统**:
    - 新增世界聊天频道，基于 PocketBase Realtime 实时通信
    - 支持折叠/展开，折叠时显示未读消息数
    - 敏感词云端过滤（3000+词库）
  - **地牢层命名系统**:
    - 每层都有独特名称，三大群系30个地名
    - 森林群系(1-10层)：荒芜旷野、黑暗丛林、蜘蛛洞穴...
    - 冰原群系(11-20层)：冰封山道、霜狼巢穴、冰晶洞窟...
    - 熔岩群系(21+层)：灼热裂隙、熔岩河谷、世界之石...
    - 周目前缀：深渊、虚空、永恒、混沌、末日
  - **传送门层数选择**:
    - 从城镇进入地牢时可选择任意已到达层数
    - 显示层数+地名，智能排序
...
- **v5.7** (2025-12-14) - 📋 **每日任务 & 仓库扩建**:
  - **每日任务系统重做**:
    - 链式解锁机制：简单→中等→困难，完成一个才显示下一个
    - 任务目标根据玩家等级动态计算（低等级简单，高等级挑战）
    - 困难任务奖励技能点，奖励金币/经验也随等级递增
    - 任务类型：击杀怪物、击杀精英、击杀BOSS、收集金币、拾取装备等
    - 每日00:00重置，左上角独立显示当前任务进度
  - **仓库扩建系统**:
    - 找瓦瑞夫可花费金币扩容仓库
    - 最多扩建3次（36→42→48→54格）
    - 费用递增：1000→3000→8000金币
  - **UI优化**:
    - 任务/公告/成就统一显示楼层名（如「第3层 枯木墓地」）

- **v5.8** (2025-12-16) - 📱 **移动端适配 & 体验优化**:
  - **游戏更名**: 正式更名为《菠萝战纪 Brawlore》
  - **移动端适配**:
    - 全面适配移动端，包括直屏设备
    - 小屏幕禁用面板拖拽，保持居中显示
    - 面板宽度自适应屏幕尺寸
  ...
  - **一键整理功能**:
    ...
    - 空位自动移至末尾
  - **交互优化**:
    - 取消入口提示和Enter键进入（改为点击）
    - 装备属性对比改为双栏气泡更直观
  - **自动战斗改进**:
    - 丢弃的物品5秒内不再拾取，避免反复丢弃

- **v5.9** (2025-12-17) - 🛒 **玩家摆摊系统**:
  - **核心功能**:
    - 在罗格营地地牢入口附近可摆摊出售装备
    - 5个摊位，先到先得
    - 设置摊位名称、选择商品、自定义价格
  ...
  - **视觉效果**:
    ...
    - 摊位名称气泡
  - **内嵌背包金币显示**:
    - 所有内嵌背包面板（商店/仓库/铁匠/摆摊）显示当前金币
    - 统一格式为"金币: xxx"

- **v6.0** (2025-12-18) - 🎁 **每日登录奖励重置 & 商店扩展**:
  - **每日登录奖励重置**:
    - Day 1: 200金币
    - Day 2: 12小时双倍金币
    - Day 3: 24小时双倍经验
...
- **v6.1** (2025-12-19) - ⚡ **性能重构 & 视觉升级**:
  - **⚡ 深度性能优化**:
    - **AI内核重构**: 全面替换 `Math.hypot` 为平方距离算法，大幅降低碰撞检测和AI寻路的CPU性能开销
    - **对象池系统**: 引入 `ParticlePool` 和 `DamageNumberPool`，减少垃圾回收(GC)卡顿，战斗更流畅
    - **UI性能升级**: 实施脏数据检查机制，仅在数值变化时更新DOM，非关键UI降频至10Hz刷新
  - **🖌️ 渲染优化**:
    - **离屏Canvas绘制**: 将已死亡怪物的血迹渲染到独立的离屏Canvas，后期战斗几百个血迹也不掉帧（性能提升巨大）
    - **反向遍历优化**: 投射物和粒子系统改用反向循环，提升批量处理效率
  - **✨ 视觉体验增强**:
    - **元素伤害反馈**:
      - ❄️ **冰冷伤害**: 敌人呈现蓝色色调，伤害数字缓慢漂浮并带有碎裂效果
      - ⚡ **闪电伤害**: 敌人闪烁黄光，伤害数字带有剧烈的电流抖动动画
      - **UI反馈**: 玩家血球/蓝球受击时增加对应的元素特效
    - **物品栏细节**:
      - 装备需求不足时，保留原有的稀有度边框颜色（暗金/绿装一眼可见），不再强制变红
      - 暗红遮罩内缩1像素，视觉层次更清晰
      
- **v6.1.1** (2025-12-20) - 体验优化 & 拾取增强:
  - **💰 自动战斗增强**:
    - **金钱优先**: 金币拾取优先级提升至最高级（与套装相同），角色会优先奔向金币
    - **全地形侦测**: 金币现在支持无视视线（LOS）侦测，即便在墙后或拐角处也能被发现并拾取
    - **智能脱困**: 增加 10 秒位置位移监控，若因掉落物位于死角导致无法拾取，AI 会自动拉黑该物品并强制脱困，彻底解决挂机卡死问题
  - **📊 视觉与精度**:
    - **XP 条清晰度**: 修改经验条百分比文字颜色为纯白并增加 4 像素黑边阴影，确保在金色背景下清晰易读
    - **浮点误差修复**: 修复击杀怪物时获得经验值显示多位小数的问题，全面采用 `Math.floor` 取整显示

- **v6.2** (2025-12-21) - 🏺 **打击感革命 & 场景破坏**:
  - **🏺 场景破坏系统**: 
    - 引入木桶、木箱、陶罐等可破坏物体，分布于地牢各处。
    - **物理交互**: 支持点击主动破碎，受击时触发震屏、碎裂粒子及专属物理音效。
    - **随机收益**: 破坏物体有概率掉落金币、药水及卷轴。
  - **💥 打击感 (Game Juice) 全面升级**:
    - **顿帧 (Hit Stop)**: 攻击命中瞬间画面短暂凝滞，强化力量感。
    - **实体缩放 (Squash & Stretch)**: 怪物受击时会产生生动的挤压拉伸动画。
    - **阶梯受击音效**: 分离普通攻击、暴击与击杀音效，战斗听感更加丰满。
  - **🎭 掉落物理渲染**:
    - **抛物线掉落**: 物品不再瞬间出现，而是带有物理轨迹的喷发而出。
    - **弹跳与影子**: 物品落地时带有写实弹跳动作，并拥有动态实时投影。
  - **🧹 性能与平衡**:
    - **智能清理**: 按品质划分掉落物消失时间（套装永久/暗金3分/稀有2分/蓝白1分），有效防止满地装备导致的帧率下降。
    - **设置开关**: 提供核心打击感特效开关，适配不同设备性能。

- **v6.3** (2025-12-21) - 🏆 **离线收益 & 成就扩展**:
  - **💤 离线挂机系统**:
    - **离线收益**: 最长8小时离线时间，收益为在线的2-3%
    - **收益计算**: 基于历史最高楼层，楼层越高收益越丰厚
    - **离线奖励**: 金币+经验+蓝/黄装备，背包满时自动转化为金币
    - **智能面板**: 上线自动弹出离线收益面板，一键领取
  - **🏆 成就系统深化**:
    - **成就大扩展**: 从6个增加到30个，涵盖六大分类
    - **分类系统**: 击杀、探索、收集、战斗、经济、成长
    - **击杀类**: 沉沦魔猎手、BOSS终结者、精英猎人、世界拯救者等
    - **探索类**: 初探地牢、地牢征服者、深渊探险家、地狱行者等
    - **收集类**: 暗金收藏家、套装收藏家、套装大师等
    - **战斗类**: 累计伤害、暴击次数、最高连击、技能使用等
    - **经济类**: 金币累计、最高强化等级等
    - **成长类**: 等级成就、天赋购买、赐福获取等

- **v6.4** (2025-12-24) - ☁️ **多端云同步 & UI精修**:
  - **☁️ 云同步优化**:
    - **同步码精简**: 从8位缩短为6位，更易记忆和输入
    - **跨设备互踢**: 多设备登录自动检测，接管会话时踢掉其他设备
    - **实时订阅**: 使用 PocketBase Realtime 实现毫秒级踢人通知
  - **🎮 用户体验优化**:
    - **新用户流程**: 调整为点击"踏入庇护所"后再选择开始方式，增加仪式感
    - **冲突处理面板**: 修复存档冲突面板不显示的问题
    - **自定义对话框**: 替换原生 alert/confirm 为游戏风格对话框
  - **🎨 界面统一**:
    - **弹窗视觉重构**: 统一全站弹窗背景、边框和阴影
    - **关闭按钮精修**: 使用 CSS 伪元素绘制叉号，完美居中
    - **统一 CSS 变量**: 引入 `--ui-panel-bg`、`--ui-border` 等全局变量

- **v6.5** (2025-12-25) - 👹 **Boss技能系统 & 周榜系统**:
  - **👹 Boss 技能系统**: 不同的 Boss 现在拥有独特的大招技能，如血鸟的毒箭齐射、女伯爵的传送新星、屠夫的低血狂暴等。
  - **🏆 周榜系统**: 引入周榜排行，每周一重置，让所有玩家都有机会登顶。

- **v6.6** (2025-12-30) - 🏆 **深渊挑战系统 & 公平竞技**:
  - **🏆 深渊竞技场**: 
    - **独立副本**: 挑战深渊守卫开启无限挑战层级。
    - **赛季制**: 每周一早8点重置榜单，根据排名发放独特奖励。
    - **公平竞技**: 挑战中禁用深渊征服者套装加成，并禁用自动战斗，确保玩家靠硬实力冲榜。
  - **🛡️ 专属奖励系统**:
    - **深渊征服者**: 全新终极绿装，拥有顶级吸血和伤害属性。
    - **头顶称号**: 全自动称号系统，BOSS/王者/前10拥有不同华丽背景称号。
  - **🔥 动态全服公告**:
    - **实况战报**: 榜单变动、新王诞生触发带红色火焰特效的全局公告。
  - **👺 NPC 进驻罗格营地**:
    - **深渊守卫**: 取代原地狱守卫，头顶实时投影本周王者的昵称信息。
  - **📱 体验优化**:
    - **专属 HUD**: 深渊中自动切换至专用精简 HUD，突出显示关键挑战参数。
