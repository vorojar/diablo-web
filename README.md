...
- **套装系统**:
  - 🟢 **塔拉夏的外袍**: 法师专属套装，强化火焰技能（2件: 能量+10, MP+50 | 4件: 火焰伤害+20, 全抗+15 | 6件: 暴击+15%, 暴伤+50%）
  - 🟢 **不朽之王**: 战士专属套装，强化物理攻击（2件: 力量+15, HP+100 | 4件: 伤害+30, 护甲+50 | 6件: 吸血+15%, 减伤+10%）
  - 🟢 **暗影舞者**: 刺客专属套装，强化敏捷和暴击（2件: 敏捷+15, 速度+20 | 4件: 暴击+10%, 闪避+15% | 6件: 攻速+30%, 暴伤+75%）
  - 套装掉落：BOSS 15%概率，精英怪 5%概率
...
- **v3.5**:
  ...
  - **🩹 Bug 修复**:
    ...
    - **地狱模式掉落修复**: 修复了地狱模式下物品掉落使用错误层数计算的问题，现在正确使用 `hellFloor` 计算套装掉落概率和BOSS掉落数量。
    - **存档信息显示**: 修复了存档信息不显示地狱模式状态的问题，现在正确显示"地狱 X层"。
    - **自动战斗箭头残留**: 修复了使用回城卷轴时，锁定目标的红色箭头会残留在罗格营地的问题。
  - **⚖️ 套装系统平衡**:
    - **塔拉夏的外袍 (6件)**: 调整为"火焰伤害 +200，法力回复 +50%，暴击率 +10%"
    - **暗影舞者 (6件)**: 修正为"敏捷 +40，伤害 +150%，暴击率 +15%"（原数值错误）
    - **套装暴击率加成**: 修复了套装加成中暴击率(critChance)不生效的问题
  - **✨ UI 增强**:
    - **濒危视觉警告**: 生命值低于20%时，屏幕边缘会呼吸式闪烁红光警告玩家
    - **立体光影**: 所有面板添加内阴影和边缘高光，模拟浮在屏幕上的实体石板效果
    - **稀有度光效**: 暗金装备金色呼吸光晕，套装装备绿色呼吸光晕，稀有装备黄色微弱光晕
    - **关键词高亮**: 物品描述中火焰/冰霜/闪电/毒素属性添加对应颜色的文字阴影
    - **统一关闭按钮**: 面板关闭按钮改为红色宝石风格圆形按钮，hover发光放大
    - **装备需求遮罩**: 需求不足的装备显示红色半透明遮罩和红色边框
    - **信息层级优化**: 数值类文本使用 Roboto 无衬线字体，提升可读性
