...
Assert-Contains -Text $game -Pattern 'function updateRenderViewport\(\)' -Message 'FAIL: resize should compute capped render viewport.'
...
Assert-Contains -Text $game -Pattern 'function clientToCanvasX\(clientX\)' -Message 'FAIL: pointer X should convert from CSS pixels to canvas pixels.'
Assert-Contains -Text $game -Pattern 'function canvasToCssX\(x\)' -Message 'FAIL: DOM overlays should convert canvas pixels back to CSS pixels.'
Assert-Contains -Text $game -Pattern 'window\.addEventListener\(''mousemove'', e => \{ mouse\.x = clientToCanvasX\(e\.clientX\); mouse\.y = clientToCanvasY\(e\.clientY\); \}\);' -Message 'FAIL: mouse movement should respect capped canvas scaling.'
Assert-Contains -Text $game -Pattern 'x: clientToCanvasX\(touch\.clientX\),' -Message 'FAIL: touch X should respect capped canvas scaling.'
...
Assert-Contains -Text $index -Pattern 'game\.js\?v=202605072230' -Message 'FAIL: index.html did not bump the game.js cache version.'
