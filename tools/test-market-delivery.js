...
(async () => {
  context.player.inventory[0] = { id: 'staged' };
  context.market.setupItems = [{ item: context.player.inventory[0] }];
  context.market.closeSetupPanel();
  assert.equal(context.player.inventory.filter(Boolean).length, 1, '取消上架不重复添加仍在背包的商品');
  ...
  context.player = { gold: 1000, inventory: [{ id: 'stock' }, null], stash: [] };
  context.SaveSystem.save = async () => { savedState = structuredClone(context.player); return true; };
  context.market.closeSetupPanel = () => {};
  context.market.showCloseStallButton = () => {};
  context.market.hideCloseStallButton = () => {};
  let fail = true;
  context.pb.send = async (url, options) => {
    if (url.endsWith('/protocol')) return { version: 2, stallReceipts: true };
    if (fail) throw new Error('模拟断网');
    return { ok: true, requestId: options.body.requestId, stallId: 'opened', items: [{ id: 'stock' }] };
  };
  await context.market.runMarketTransaction('open-stall', { sellerId: 'buyer', items: [{ item: { id: 'stock' }, price: 10 }] }, 500);
  assert.equal(context.player.gold, 500);
  assert.equal(context.player.inventory.filter(Boolean).length, 0);
  assert.equal(savedState.marketPending.kind, 'open-stall');
  context.player = structuredClone(savedState); fail = false;
  await context.market.recoverPendingTransactions();
  assert.equal(context.player.gold, 500); assert.equal(context.market.localStallId, 'opened');
  fail = true;
  await context.market.closeStall();
  assert.equal(context.player.inventory.filter(Boolean).length, 0, '断网不提前返还');
  assert.equal(context.player.marketPending.kind, 'close-stall');
  fail = false;
  context.player.inventory = [{ id: 'loot1' }, { id: 'loot2' }];
  await context.market.recoverPendingTransactions();
  assert.equal(context.player.marketPending.applied, false, '满包保留待交付');
  context.player.inventory[1] = null;
  await context.market.recoverPendingTransactions();
  assert.equal(context.player.inventory[1].id, 'stock');
  context.player = structuredClone(savedState);
  await context.market.recoverPendingTransactions();
  assert.equal(context.player.inventory.filter(i => i && i.id === 'stock').length, 1);
  console.log('PASS 上架预存、断网恢复、收摊断网不返还、满包补领与重启不重复');
